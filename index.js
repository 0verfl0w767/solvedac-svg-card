/**
 *    ____                  __________
 *   / __ \_   _____  _____/ __/ / __ \_      __
 *  / / / / | / / _ \/ ___/ /_/ / / / / | /| / /
 * / /_/ /| |/ /  __/ /  / __/ / /_/ /| |/ |/ /
 * \____/ |___/\___/_/  /_/ /_/\____/ |__/|__/
 *
 * The copyright indication and this authorization indication shall be
 * recorded in all copies or in important parts of the Software.
 *
 * @github https://github.com/0verfl0w767
 *
 */
const express = require('express')
const {param, query, validationResult} = require('express-validator')
const fs = require('fs')
const request = require('request-promise')
const http = require('http')
const https = require('https')
const {dirname} = require('path')

const app = express()

const parametersValidation = (value) => {
  const allowedParameters = ['username', 'bgc1', 'bgc2', 'bgc3']
  return Object.keys(value).every((key) => allowedParameters.includes(key))
}

app.get(
  '/info',
  query().custom(parametersValidation),
  query('username').exists().trim(),
  query('bgc1').optional().trim(),
  query('bgc2').optional().trim(),
  query('bgc3').optional().trim(),
  async (req, res) => {
    const errorFormatter = ({location, msg, param, value, nestedErrors}) => {
      return param + ': ' + msg
    }
    const result = validationResult(req).formatWith(errorFormatter)
    if (!result.isEmpty()) {
      res.status(401).json(result)
      return
    }
    const username = req.query.username
    const bgc1 = req.query.bgc1 == undefined ? 'yellow' : '#' + req.query.bgc1
    const bgc2 = req.query.bgc2 == undefined ? 'deepskyblue' : '#' + req.query.bgc2
    const bgc3 = req.query.bgc3 == undefined ? 'hotpink' : '#' + req.query.bgc3
    console.log(bgc1, bgc2, bgc3)
    try {
      const options1 = {
        uri: 'https://solved.ac/api/v3/user/show?handle=' + username,
        method: 'GET',
        json: true,
      }
      // const data1 = await request(options1, (error, res, body) => {
      //   if (!error) {
      //     return res
      //   }
      // })
      const data1 = await fetch('https://solved.ac/api/v3/user/show?handle=' + username).then((res) => res.json())
      //console.log(data1)
      const options2 = {
        uri: 'https://solved.ac/api/v3/user/organizations?handle=' + username,
        method: 'GET',
        json: true,
      }
      // const data2 = await request(options2, (error, res, body) => {
      //   if (!error) {
      //     return res
      //   }
      // })
      const tierList = {
        1: 'Bronze V',
        2: 'Bronze IV',
        3: 'Bronze III',
        4: 'Bronze II',
        5: 'Bronze I',
        6: 'Silver V',
        7: 'Silver IV',
        8: 'Silver III',
        9: 'Silver II',
        10: 'Silver I',
        11: 'Gold V',
        12: 'Gold IV',
        13: 'Gold III',
        14: 'Gold II',
        15: 'Gold I',
        16: 'Platinum V',
        17: 'Platinum IV',
        18: 'Platinum III',
        19: 'Platinum II',
        20: 'Platinum I',
        21: 'Diamond V',
        22: 'Diamond IV',
        23: 'Diamond III',
        24: 'Diamond II',
        25: 'Diamond I',
        26: 'Ruby V',
        27: 'Ruby IV',
        28: 'Ruby III',
        29: 'Ruby II',
        30: 'Ruby I',
        31: 'Master',
      }
      const classSVGList = {
        0: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#2d2d2d;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="45.366" y1="60" x2="74.634" y2="60" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c0</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><path class="cls-6" d="M60.021,77.872a13.529,13.529,0,0,1-5.982-1.323,13.679,13.679,0,0,1-4.662-3.735A17.3,17.3,0,0,1,46.4,67.149a25.329,25.329,0,0,1,0-14.34,17.084,17.084,0,0,1,2.98-5.649,13.81,13.81,0,0,1,4.657-3.709,13.653,13.653,0,0,1,16.605,3.715A17.317,17.317,0,0,1,73.6,52.809a25.329,25.329,0,0,1,0,14.34,17.54,17.54,0,0,1-2.95,5.658,13.425,13.425,0,0,1-4.671,3.744A13.563,13.563,0,0,1,60.021,77.872Zm0-25.537a2.025,2.025,0,0,0-1.238.389,4.21,4.21,0,0,0-1.172,1.386,9.034,9.034,0,0,0-.884,2.487,17.411,17.411,0,0,0,0,6.783,9.333,9.333,0,0,0,.89,2.5,4.139,4.139,0,0,0,1.169,1.4,2.022,2.022,0,0,0,1.236.389,2.046,2.046,0,0,0,1.228-.4A3.878,3.878,0,0,0,62.4,65.9a9.868,9.868,0,0,0,.879-2.517,17.411,17.411,0,0,0,0-6.783,9.548,9.548,0,0,0-.874-2.506,3.909,3.909,0,0,0-1.151-1.36A2.08,2.08,0,0,0,60.021,52.335Z"/><path class="cls-7" d="M60.021,43.665a11.852,11.852,0,0,1,9.427,4.468,15.72,15.72,0,0,1,2.686,5.144,23.806,23.806,0,0,1,0,13.405,15.952,15.952,0,0,1-2.686,5.165,11.842,11.842,0,0,1-4.139,3.32,12.073,12.073,0,0,1-14.737-3.32,15.717,15.717,0,0,1-2.706-5.165,23.806,23.806,0,0,1,0-13.4,15.49,15.49,0,0,1,2.706-5.144,12.212,12.212,0,0,1,4.14-3.3,11.97,11.97,0,0,1,5.309-1.168m0,25.538a3.593,3.593,0,0,0,2.111-.677,5.36,5.36,0,0,0,1.619-1.906,11.333,11.333,0,0,0,1.024-2.91,18.88,18.88,0,0,0,0-7.439,11.018,11.018,0,0,0-1.024-2.912,5.414,5.414,0,0,0-1.619-1.885,3.667,3.667,0,0,0-4.243,0,5.685,5.685,0,0,0-1.619,1.885,10.506,10.506,0,0,0-1.045,2.912,18.88,18.88,0,0,0,0,7.439,10.794,10.794,0,0,0,1.045,2.91,5.623,5.623,0,0,0,1.619,1.906,3.562,3.562,0,0,0,2.132.677m0-28.612a15.067,15.067,0,0,0-6.655,1.478,15.326,15.326,0,0,0-5.174,4.118,18.588,18.588,0,0,0-3.254,6.155,26.857,26.857,0,0,0,0,15.276,18.842,18.842,0,0,0,3.245,6.164,15.381,15.381,0,0,0,18.463,4.153,14.936,14.936,0,0,0,5.2-4.168,19.07,19.07,0,0,0,3.213-6.149,26.857,26.857,0,0,0,0-15.276A18.823,18.823,0,0,0,71.84,46.2a14.929,14.929,0,0,0-11.819-5.61Zm0,25.538a.507.507,0,0,1-.34-.1,2.647,2.647,0,0,1-.718-.889,7.842,7.842,0,0,1-.735-2.088,15.875,15.875,0,0,1,0-6.128,7.529,7.529,0,0,1,.724-2.063,2.687,2.687,0,0,1,.728-.888.507.507,0,0,1,.34-.1.54.54,0,0,1,.347.12,2.413,2.413,0,0,1,.681.834,8.037,8.037,0,0,1,.722,2.1,15.9,15.9,0,0,1,0,6.127,8.416,8.416,0,0,1-.735,2.125,2.39,2.39,0,0,1-.675.837.535.535,0,0,1-.342.117Z"/></g></g></g></svg>',
        1: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#1d7edd;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="59.219" y1="78.535" x2="59.219" y2="44.553" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c1</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-6" points="55.919 78.535 55.919 54.432 51.41 54.432 51.41 44.553 67.028 44.553 67.028 78.535 55.919 78.535"/><path class="cls-7" d="M65.491,46.09V77H57.456V52.9H52.947v-6.8H65.491m3.074-3.075H49.873V55.969h4.509v24.1H68.565V43.015Z"/></g></g></g></svg>',
        2: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#1ab3e2;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="58.612" y1="78.535" x2="58.612" y2="43.733" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c2</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><path class="cls-6" d="M43.594,78.535,53.77,67.979c1.977-2.058,3.543-3.819,4.653-5.232a28.356,28.356,0,0,0,2.427-3.506,8.166,8.166,0,0,0,.887-2.041,6.31,6.31,0,0,0,.17-1.272,2.607,2.607,0,0,0-.143-.856,1.926,1.926,0,0,0-.377-.652,1.877,1.877,0,0,0-.6-.448,1.911,1.911,0,0,0-.828-.155,1.655,1.655,0,0,0-1.4.535,2.423,2.423,0,0,0-.547,1.658A8.361,8.361,0,0,0,58.12,57.1l.251,1.754H46.289V57.321a14.753,14.753,0,0,1,.991-5.488A12.745,12.745,0,0,1,50.1,47.5a12.525,12.525,0,0,1,4.317-2.8,14.646,14.646,0,0,1,5.378-.967,15.144,15.144,0,0,1,5.314.917,13,13,0,0,1,4.3,2.607,12.1,12.1,0,0,1,2.863,4.009A12.3,12.3,0,0,1,73.3,56.3a11.846,11.846,0,0,1-1.251,5.386,18.114,18.114,0,0,1-3.778,4.829l-2.29,2.144H73.63v9.879Z"/><path class="cls-7" d="M59.8,45.27a13.525,13.525,0,0,1,4.776.82,11.419,11.419,0,0,1,3.792,2.3,10.543,10.543,0,0,1,2.5,3.5,10.733,10.733,0,0,1,.9,4.407,10.278,10.278,0,0,1-1.086,4.693A16.541,16.541,0,0,1,67.215,65.4l-5.124,4.8h10V77H47.211l7.666-7.952q3.033-3.158,4.755-5.35a29.625,29.625,0,0,0,2.562-3.71,9.8,9.8,0,0,0,1.045-2.459,7.789,7.789,0,0,0,.205-1.6,4.174,4.174,0,0,0-.225-1.353,3.484,3.484,0,0,0-.677-1.168A3.277,3.277,0,0,0,59.96,52.28,3.208,3.208,0,0,0,57.4,53.345a3.929,3.929,0,0,0-.923,2.665,10.03,10.03,0,0,0,.123,1.311H47.826a13.176,13.176,0,0,1,.881-4.918,11.163,11.163,0,0,1,2.48-3.813,10.968,10.968,0,0,1,3.792-2.459A13.071,13.071,0,0,1,59.8,45.27m0-3.074a16.192,16.192,0,0,0-5.94,1.073,14.063,14.063,0,0,0-4.841,3.147,14.245,14.245,0,0,0-3.163,4.85,16.3,16.3,0,0,0-1.1,6.055V60.4H58.271q-.47.654-1.058,1.4c-1.079,1.374-2.611,3.1-4.553,5.117L45,74.864l-5.021,5.208h35.19V67.119H69.853a18.91,18.91,0,0,0,3.57-4.743A13.37,13.37,0,0,0,74.84,56.3a13.835,13.835,0,0,0-1.167-5.654,13.648,13.648,0,0,0-3.225-4.517,14.539,14.539,0,0,0-4.8-2.916A16.663,16.663,0,0,0,59.8,42.2Zm-.09,13.181a1.112,1.112,0,0,1,.255-.023.825.825,0,0,1,.194.016.376.376,0,0,1,.083.071.39.39,0,0,1,.075.136,1.072,1.072,0,0,1,.057.351,4.754,4.754,0,0,1-.135.946,5.04,5.04,0,0,1-.441,1.078l-.151-1.063a7.183,7.183,0,0,1-.092-.879.941.941,0,0,1,.155-.633Z"/></g></g></g></svg>',
        3: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#16d671;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="59.673" y1="79.396" x2="59.673" y2="43.692" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c3</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><path class="cls-6" d="M59.754,79.4a16.135,16.135,0,0,1-7.107-1.548A12.079,12.079,0,0,1,47.391,72.8a10.4,10.4,0,0,1-1.217-3.169,15.792,15.792,0,0,1-.3-3.043V65.048H56.85l.055,1.48A3.118,3.118,0,0,0,57.7,68.77a2.966,2.966,0,0,0,2.14.706,2.5,2.5,0,0,0,1.948-.611,2.68,2.68,0,0,0,.614-1.828,2.187,2.187,0,0,0-.655-1.87,2.646,2.646,0,0,0-1.825-.651,3.292,3.292,0,0,0-.526.04q-.359.06-.8.14l-1.812.33V56.891h-9.7l.2-1.717a14.824,14.824,0,0,1,1.354-4.8,11.869,11.869,0,0,1,2.711-3.645,11.4,11.4,0,0,1,3.88-2.275,15.375,15.375,0,0,1,9.427-.045,10.921,10.921,0,0,1,3.716,2.1,9.428,9.428,0,0,1,2.447,3.375,10.832,10.832,0,0,1,.844,4.319,8.554,8.554,0,0,1-1.056,4.413,8.387,8.387,0,0,1-.936,1.278,8.14,8.14,0,0,1,2.185,2.139,9.528,9.528,0,0,1,1.61,5.574,10.948,10.948,0,0,1-3.974,8.6,13.206,13.206,0,0,1-4.357,2.37A17.14,17.14,0,0,1,59.754,79.4ZM58.51,56.616a3.082,3.082,0,0,0,.363.03h.39a1.85,1.85,0,0,0,1.4-.477,1.533,1.533,0,0,0,.468-1.225,1.457,1.457,0,0,0-.419-1.155,1.575,1.575,0,0,0-1.2-.423,1.519,1.519,0,0,0-1.244.485,2.037,2.037,0,0,0-.5,1.5v1.169Z"/><path class="cls-7" d="M60,45.229a12.836,12.836,0,0,1,4.16.636,9.353,9.353,0,0,1,3.2,1.8,7.856,7.856,0,0,1,2.05,2.829,9.263,9.263,0,0,1,.717,3.709,7.026,7.026,0,0,1-.84,3.628,7.545,7.545,0,0,1-2.4,2.439,6.852,6.852,0,0,1,3.71,2.644,8,8,0,0,1,1.332,4.693,9.411,9.411,0,0,1-3.423,7.42,11.628,11.628,0,0,1-3.853,2.091,15.543,15.543,0,0,1-4.9.737,14.549,14.549,0,0,1-6.435-1.393,10.525,10.525,0,0,1-4.591-4.427,8.843,8.843,0,0,1-1.045-2.706,14.062,14.062,0,0,1-.267-2.746h7.953a4.582,4.582,0,0,0,1.27,3.3,4.453,4.453,0,0,0,3.2,1.127,3.887,3.887,0,0,0,3.116-1.148,4.193,4.193,0,0,0,.984-2.829A3.79,3.79,0,0,0,62.747,64a4.2,4.2,0,0,0-2.828-1.025,4.751,4.751,0,0,0-.78.062c-.245.041-.519.089-.819.143V58.142a4.643,4.643,0,0,0,.553.041h.39a3.409,3.409,0,0,0,2.459-.9,3.087,3.087,0,0,0,.943-2.336,3.014,3.014,0,0,0-.881-2.255,3.124,3.124,0,0,0-2.275-.86,3.071,3.071,0,0,0-2.378.983,3.613,3.613,0,0,0-.9,2.542H48.81a13.289,13.289,0,0,1,1.209-4.3,10.307,10.307,0,0,1,2.357-3.177,9.8,9.8,0,0,1,3.361-1.967A12.8,12.8,0,0,1,60,45.229m0-3.074a15.921,15.921,0,0,0-5.282.85,12.908,12.908,0,0,0-4.4,2.583A13.405,13.405,0,0,0,47.257,49.7a16.387,16.387,0,0,0-1.5,5.294l-.406,3.435h9.894v5.083h-10.9v3.075a17.251,17.251,0,0,0,.325,3.342,11.9,11.9,0,0,0,1.4,3.644,13.567,13.567,0,0,0,5.913,5.659,17.649,17.649,0,0,0,7.777,1.7,18.694,18.694,0,0,0,5.863-.893,14.738,14.738,0,0,0,4.86-2.65A12.486,12.486,0,0,0,75,67.61a11.035,11.035,0,0,0-1.887-6.454A9.852,9.852,0,0,0,71.8,59.623c.045-.072.09-.145.133-.219a10.056,10.056,0,0,0,1.272-5.2,12.346,12.346,0,0,0-.97-4.928,10.937,10.937,0,0,0-2.844-3.921,12.469,12.469,0,0,0-4.232-2.4,15.983,15.983,0,0,0-5.153-.8Zm-.62,12.756a.863.863,0,0,1,.128-.008q.048,0,.081,0v.038a.967.967,0,0,1-.007.12.708.708,0,0,1-.265.043.526.526,0,0,1,.062-.2Zm-.947,11.376.435-.079c.285-.051.544-.1.777-.136a1.676,1.676,0,0,1,.273-.019,1.125,1.125,0,0,1,.822.28,1.342,1.342,0,0,1,.12.7,1.165,1.165,0,0,1-.215.8,2.259,2.259,0,0,1-.81.108,1.584,1.584,0,0,1-1.081-.284,1.931,1.931,0,0,1-.314-1.185l-.007-.183Z"/></g></g></g></svg>',
        4: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#23c91b;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="59.693" y1="78.535" x2="59.693" y2="44.553" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c4</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><path class="cls-6" d="M60.021,78.535V72.181H45.1V65.048l12.479-20.5H70.555V62.958h3.731v9.223H70.555v6.354Zm0-15.577V58.014l-2.88,4.944Z"/><path class="cls-7" d="M69.018,46.09V64.5h3.731v6.149H69.018V77h-7.46V70.644H46.637V65.479L58.443,46.09H69.018M54.467,64.5h7.091V52.321L54.467,64.5m17.626-21.48H56.716l-.9,1.476L44.011,63.88l-.449.736v9.1H58.484v6.354H72.093V73.718h3.73v-12.3h-3.73V43.015Z"/></g></g></g></svg>',
        5: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:#99d111;}.cls-3{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-3,.cls-4{mix-blend-mode:overlay;}.cls-4{fill:url(#무제_그라디언트_3-2);}.cls-5{fill:url(#무제_그라디언트_130);}.cls-6{fill:#fff;}.cls-7{fill:none;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="59.685" y1="79.396" x2="59.685" y2="44.553" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c5</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-3" points="10 60 60 110 110 60 10 60"/><rect class="cls-4" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><path class="cls-5" d="M58.278,79.4a22.037,22.037,0,0,1-5.706-.729,18.842,18.842,0,0,1-5.155-2.254l-.857-.539,1.775-11.269.049.042,3.127-20.094H70.556v9.879H59.635l-.023.144c.262-.014.529-.021.8-.021a12.978,12.978,0,0,1,4.926.933,11.616,11.616,0,0,1,6.54,6.54A13.1,13.1,0,0,1,72.811,67a12.252,12.252,0,0,1-1.063,5.15A11.278,11.278,0,0,1,68.737,76.1a13.31,13.31,0,0,1-4.629,2.463A19.5,19.5,0,0,1,58.278,79.4ZM50.427,66.4a15.039,15.039,0,0,0,3.789,2.439,8.735,8.735,0,0,0,3.448.8,4,4,0,0,0,2.848-.922,2.785,2.785,0,0,0,.944-2.174,2.874,2.874,0,0,0-1.3-2.469,6.871,6.871,0,0,0-4.133-1.117,12.576,12.576,0,0,0-2.556.275,12.254,12.254,0,0,0-2.586.857l-2.055.938Z"/><path class="cls-6" d="M69.019,46.09V52.9h-10.7l-.532,3.4c.491-.054.943-.1,1.352-.143s.834-.062,1.271-.062a11.377,11.377,0,0,1,4.345.82,10.067,10.067,0,0,1,5.7,5.7A11.507,11.507,0,0,1,71.273,67a10.7,10.7,0,0,1-.922,4.509,9.727,9.727,0,0,1-2.6,3.423,11.744,11.744,0,0,1-4.1,2.172,17.887,17.887,0,0,1-5.371.758,20.4,20.4,0,0,1-5.308-.676,17.22,17.22,0,0,1-4.734-2.07l1.189-7.543a16.472,16.472,0,0,0,4.181,2.685,10.217,10.217,0,0,0,4.058.923,5.5,5.5,0,0,0,3.894-1.332,4.32,4.32,0,0,0,1.435-3.3,4.393,4.393,0,0,0-1.926-3.71,8.321,8.321,0,0,0-5.042-1.414,14.06,14.06,0,0,0-2.871.307,13.755,13.755,0,0,0-2.91.964l2.583-16.6H69.019m3.074-3.075h-21.9l-.405,2.6-2.5,16.06-.044-.037-.859,5.451L45.2,74.634l-.316,2L46.6,77.714a20.377,20.377,0,0,0,5.576,2.438,23.573,23.573,0,0,0,6.1.781,21.057,21.057,0,0,0,6.29-.9,14.834,14.834,0,0,0,5.159-2.754,12.815,12.815,0,0,0,3.418-4.493A13.779,13.779,0,0,0,74.348,67,14.61,14.61,0,0,0,73.3,61.448a13.426,13.426,0,0,0-2.925-4.454,13.1,13.1,0,0,0-1.157-1.025h2.876V43.015ZM51.654,65.429a10.632,10.632,0,0,1,2.127-.691,11.07,11.07,0,0,1,2.244-.243,5.408,5.408,0,0,1,3.222.819,1.343,1.343,0,0,1,.672,1.231,1.264,1.264,0,0,1-.452,1.046,2.384,2.384,0,0,1-1.8.511,7.232,7.232,0,0,1-2.836-.669,13.274,13.274,0,0,1-3.174-2Z"/><rect class="cls-7" width="120" height="120"/></g></g></g></svg>',
        6: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#e5bb0c;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="60.021" y1="79.396" x2="60.021" y2="44.553" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c6</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><path class="cls-6" d="M60.041,79.4a17.234,17.234,0,0,1-6.03-1A14.033,14.033,0,0,1,49.39,75.63a12.2,12.2,0,0,1-2.973-4.147,12.471,12.471,0,0,1-1.03-5.02,17.552,17.552,0,0,1,1.269-6.226,23.5,23.5,0,0,1,3.833-6.431l7.639-9.253h13.4l-7.939,9.128a11.206,11.206,0,0,1,3.933.971,11.851,11.851,0,0,1,3.766,2.708,12.417,12.417,0,0,1,2.48,4,13.377,13.377,0,0,1,.883,4.861,13.07,13.07,0,0,1-1.025,5.175A12.292,12.292,0,0,1,70.657,75.6,13.856,13.856,0,0,1,66.03,78.4,17.123,17.123,0,0,1,60.041,79.4ZM60,62.794a3.287,3.287,0,0,0-1.363.278,3.765,3.765,0,0,0-1.946,1.944,3.493,3.493,0,0,0,0,2.729,3.768,3.768,0,0,0,1.947,1.945,3.485,3.485,0,0,0,2.728,0,3.768,3.768,0,0,0,1.157-.79,3.806,3.806,0,0,0,.79-1.157,3.506,3.506,0,0,0,0-2.727,3.8,3.8,0,0,0-.789-1.156,3.763,3.763,0,0,0-1.156-.788A3.3,3.3,0,0,0,60,62.794Z"/><path class="cls-7" d="M68.158,46.09,59.385,56.174h.164a6.256,6.256,0,0,1,1.886-.841,7.009,7.009,0,0,1,1.311-.143,9.85,9.85,0,0,1,4.141.861,10.262,10.262,0,0,1,3.279,2.357,10.848,10.848,0,0,1,2.173,3.5,11.806,11.806,0,0,1,.778,4.3,11.51,11.51,0,0,1-.9,4.57,10.742,10.742,0,0,1-2.6,3.69,12.284,12.284,0,0,1-4.119,2.48,17.073,17.073,0,0,1-10.946,0,12.453,12.453,0,0,1-4.12-2.46,10.614,10.614,0,0,1-2.6-3.628,10.892,10.892,0,0,1-.9-4.406,15.983,15.983,0,0,1,1.168-5.677,21.832,21.832,0,0,1,3.587-6.006l7.173-8.69h9.306M60,71.505a4.846,4.846,0,0,0,1.988-.41,5.309,5.309,0,0,0,2.727-2.726,5.036,5.036,0,0,0,0-3.976,5.309,5.309,0,0,0-2.727-2.726,5.025,5.025,0,0,0-3.976,0,5.312,5.312,0,0,0-2.726,2.726,5.025,5.025,0,0,0,0,3.976A5.312,5.312,0,0,0,58.012,71.1a4.842,4.842,0,0,0,1.988.41m14.907-28.49H57.4l-.923,1.118-7.173,8.69a25.025,25.025,0,0,0-4.087,6.864,19.089,19.089,0,0,0-1.372,6.776A14.006,14.006,0,0,0,45.008,72.1a13.723,13.723,0,0,0,3.344,4.667,15.582,15.582,0,0,0,5.123,3.073,20.114,20.114,0,0,0,13.092,0A15.386,15.386,0,0,0,71.7,76.73,13.832,13.832,0,0,0,75.043,72a14.614,14.614,0,0,0,1.149-5.778,14.907,14.907,0,0,0-.987-5.416,13.944,13.944,0,0,0-2.788-4.487,13.373,13.373,0,0,0-4.253-3.06,12.318,12.318,0,0,0-1.634-.609l3.947-4.537,4.43-5.093ZM60,68.431a1.792,1.792,0,0,1-.739-.145,2.257,2.257,0,0,1-.693-.47,2.232,2.232,0,0,1-.471-.693,1.958,1.958,0,0,1,0-1.479,2.241,2.241,0,0,1,1.165-1.168,1.96,1.96,0,0,1,1.482,0,2.231,2.231,0,0,1,1.165,1.168,1.967,1.967,0,0,1,0,1.48,2.245,2.245,0,0,1-1.164,1.161,1.8,1.8,0,0,1-.74.145Z"/></g></g></g></svg>',
        7: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#ef9e0f;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="61.42" y1="78.535" x2="61.42" y2="44.553" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c7</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-6" points="45.473 78.535 59.574 54.432 47.806 54.432 47.806 44.553 77.368 44.553 57.899 78.535 45.473 78.535"/><path class="cls-7" d="M74.716,46.09,57.009,77H48.154l14.1-24.1H49.343v-6.8H74.716m5.305-3.075H46.268V55.969H56.894L45.5,75.445l-2.707,4.627h16l.886-1.546L77.384,47.618l2.637-4.6Z"/></g></g></g></svg>',
        8: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#ff6700;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="60" y1="79.354" x2="60" y2="43.692" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c8</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><path class="cls-6" d="M60,79.354a17.036,17.036,0,0,1-5.113-.747,13.536,13.536,0,0,1-4.228-2.154,10.612,10.612,0,0,1-2.917-3.4,9.54,9.54,0,0,1,3.333-12.467,8.168,8.168,0,0,1-3.147-6.706,9.474,9.474,0,0,1,.937-4.225,9.9,9.9,0,0,1,2.546-3.226,11.24,11.24,0,0,1,3.842-2.049,16.764,16.764,0,0,1,9.493,0,11.231,11.231,0,0,1,3.844,2.048,9.934,9.934,0,0,1,2.545,3.225,9.479,9.479,0,0,1,.937,4.227,10.406,10.406,0,0,1-.294,2.574,6.568,6.568,0,0,1-1.1,2.3,8.642,8.642,0,0,1-1.889,1.824l-.035.026a9.15,9.15,0,0,1,4.586,8.034A9.283,9.283,0,0,1,72.284,73a10.47,10.47,0,0,1-2.874,3.407,13.618,13.618,0,0,1-4.2,2.177A16.92,16.92,0,0,1,60,79.354ZM60,65.5a2.007,2.007,0,0,0-1.525.626,2.057,2.057,0,0,0-.627,1.526,2.356,2.356,0,0,0,.637,1.62,2.185,2.185,0,0,0,.7.485,1.936,1.936,0,0,0,.82.171,1.834,1.834,0,0,0,.776-.17,2.471,2.471,0,0,0,.742-.508,2.178,2.178,0,0,0,.469-.7,2.141,2.141,0,0,0,.166-.855,2.074,2.074,0,0,0-.637-1.557A2.031,2.031,0,0,0,60,65.5Zm0-13.363a1.749,1.749,0,0,0-1.333.535,1.7,1.7,0,0,0-.532,1.289,1.837,1.837,0,0,0,.544,1.364A1.717,1.717,0,0,0,60,55.866a1.617,1.617,0,0,0,.7-.141,1.984,1.984,0,0,0,.61-.431,2.154,2.154,0,0,0,.416-.633,1.647,1.647,0,0,0,.139-.66,1.761,1.761,0,0,0-.544-1.321A1.717,1.717,0,0,0,60,52.136Z"/><path class="cls-7" d="M60,45.229a14.163,14.163,0,0,1,4.284.615,9.648,9.648,0,0,1,3.32,1.762,8.35,8.35,0,0,1,2.153,2.727,7.919,7.919,0,0,1,.778,3.545,8.882,8.882,0,0,1-.246,2.193,5,5,0,0,1-.84,1.763,7.113,7.113,0,0,1-1.558,1.5,14.132,14.132,0,0,1-2.4,1.353q6.31,2.46,6.312,7.952a7.736,7.736,0,0,1-.882,3.649,8.929,8.929,0,0,1-2.459,2.91,12.042,12.042,0,0,1-3.73,1.927,16.4,16.4,0,0,1-9.387.02A11.977,11.977,0,0,1,51.6,75.235a9.038,9.038,0,0,1-2.5-2.91,7.661,7.661,0,0,1-.9-3.69q0-5.615,6.109-7.952-4.839-2.007-4.838-6.8a7.906,7.906,0,0,1,.779-3.545A8.334,8.334,0,0,1,52.4,47.606a9.666,9.666,0,0,1,3.32-1.762A14.167,14.167,0,0,1,60,45.229M60,57.4a3.157,3.157,0,0,0,1.352-.287,3.511,3.511,0,0,0,1.067-.759,3.678,3.678,0,0,0,.717-1.086A3.157,3.157,0,0,0,63.4,54a3.3,3.3,0,0,0-.983-2.4A3.254,3.254,0,0,0,60,50.6a3.289,3.289,0,0,0-2.419.984A3.243,3.243,0,0,0,56.6,53.96a3.357,3.357,0,0,0,.983,2.439A3.254,3.254,0,0,0,60,57.4m0,14.06a3.375,3.375,0,0,0,1.414-.308,4.011,4.011,0,0,0,1.189-.819,3.721,3.721,0,0,0,.8-1.189,3.677,3.677,0,0,0,.288-1.456A3.6,3.6,0,0,0,62.6,65.048a3.68,3.68,0,0,0-6.293,2.6,3.9,3.9,0,0,0,1.067,2.686,3.745,3.745,0,0,0,1.168.819A3.478,3.478,0,0,0,60,71.464m0-29.309a17.332,17.332,0,0,0-5.21.757,12.749,12.749,0,0,0-4.364,2.335,11.432,11.432,0,0,0-2.939,3.724,11,11,0,0,0-1.1,4.907,9.825,9.825,0,0,0,2.319,6.553,10.572,10.572,0,0,0-3.59,8.2,10.748,10.748,0,0,0,1.269,5.146,12.117,12.117,0,0,0,3.334,3.891,15.072,15.072,0,0,0,4.7,2.4,19.449,19.449,0,0,0,11.255-.028,15.151,15.151,0,0,0,4.674-2.427,12.024,12.024,0,0,0,3.292-3.908,10.809,10.809,0,0,0,1.234-5.076A10.424,10.424,0,0,0,71.214,60.5a9.308,9.308,0,0,0,.7-.831,8.1,8.1,0,0,0,1.352-2.837,11.967,11.967,0,0,0,.342-2.954,11,11,0,0,0-1.095-4.9,11.451,11.451,0,0,0-2.939-3.726,12.749,12.749,0,0,0-4.367-2.336A17.316,17.316,0,0,0,60,42.155Zm0,12.174c-.143,0-.153-.011-.223-.081a.312.312,0,0,1-.105-.288.2.2,0,0,1,.083-.2c.064-.064.084-.083.245-.083.143,0,.156.012.224.082a.262.262,0,0,1,.1.245.65.65,0,0,1-.13.232.46.46,0,0,1-.138.094h0c-.006,0-.026,0-.057,0ZM60,68.39a.406.406,0,0,1-.183-.033.658.658,0,0,1-.218-.145.764.764,0,0,1-.158-.253.779.779,0,0,1-.056-.308.526.526,0,0,1,.186-.448A.481.481,0,0,1,60,67.036a.5.5,0,0,1,.431.188.548.548,0,0,1,.185.468.6.6,0,0,1-.042.25.667.667,0,0,1-.144.221.954.954,0,0,1-.287.194A.314.314,0,0,1,60,68.39Z"/></g></g></g></svg>',
        9: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#ef165e;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="59.979" y1="78.535" x2="59.979" y2="43.692" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient></defs><title>c9</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><path class="cls-6" d="M48.468,78.535l7.941-9.128a11.462,11.462,0,0,1-7.728-3.689,12.551,12.551,0,0,1-2.453-3.988,13.4,13.4,0,0,1-.882-4.859A13.1,13.1,0,0,1,46.37,51.7a12.326,12.326,0,0,1,2.972-4.213,13.891,13.891,0,0,1,4.629-2.794,18.586,18.586,0,0,1,12.017,0,14.067,14.067,0,0,1,4.621,2.767A12.225,12.225,0,0,1,73.584,51.6a12.489,12.489,0,0,1,1.029,5.021,17.547,17.547,0,0,1-1.269,6.226,23.449,23.449,0,0,1-3.833,6.431l-7.639,9.253ZM60,53.12a3.3,3.3,0,0,0-1.364.278,3.78,3.78,0,0,0-1.945,1.943,3.494,3.494,0,0,0,0,2.727,3.775,3.775,0,0,0,1.946,1.947,3.479,3.479,0,0,0,2.728,0,3.754,3.754,0,0,0,1.158-.788,3.792,3.792,0,0,0,.79-1.159,3.5,3.5,0,0,0,0-2.728A3.784,3.784,0,0,0,61.364,53.4,3.307,3.307,0,0,0,60,53.12Z"/><path class="cls-7" d="M59.959,45.229a15.652,15.652,0,0,1,5.493.9,12.453,12.453,0,0,1,4.119,2.459,10.646,10.646,0,0,1,2.6,3.628,10.9,10.9,0,0,1,.9,4.407A15.957,15.957,0,0,1,71.908,62.3a21.788,21.788,0,0,1-3.587,6.005L61.148,77h-9.3l8.773-10.084h-.165a6.206,6.206,0,0,1-1.885.84,6.944,6.944,0,0,1-1.312.144,9.967,9.967,0,0,1-7.44-3.218,11.009,11.009,0,0,1-2.153-3.505,11.806,11.806,0,0,1-.778-4.3,11.5,11.5,0,0,1,.9-4.571,10.721,10.721,0,0,1,2.6-3.689,12.269,12.269,0,0,1,4.12-2.48,15.51,15.51,0,0,1,5.451-.9M60,61.831a4.85,4.85,0,0,0,1.989-.41A5.312,5.312,0,0,0,64.715,58.7a5.038,5.038,0,0,0,0-3.977,5.315,5.315,0,0,0-2.726-2.725,5.025,5.025,0,0,0-3.976,0,5.312,5.312,0,0,0-2.727,2.725,5.027,5.027,0,0,0,0,3.977,5.309,5.309,0,0,0,2.727,2.726,4.834,4.834,0,0,0,1.987.41m-.041-19.676a18.644,18.644,0,0,0-6.525,1.095A15.4,15.4,0,0,0,48.3,46.356a13.836,13.836,0,0,0-3.34,4.737,14.621,14.621,0,0,0-1.148,5.778,14.923,14.923,0,0,0,.986,5.416,14.088,14.088,0,0,0,2.756,4.473,13.025,13.025,0,0,0,4.277,3.078,12.4,12.4,0,0,0,1.641.607L49.523,74.98l-4.43,5.092H62.6l.922-1.117,7.173-8.69A25.018,25.018,0,0,0,74.78,63.4a19.086,19.086,0,0,0,1.371-6.775,14.022,14.022,0,0,0-1.158-5.635,13.768,13.768,0,0,0-3.344-4.666,15.6,15.6,0,0,0-5.122-3.074,18.8,18.8,0,0,0-6.568-1.095ZM60,58.757a1.779,1.779,0,0,1-.737-.145A2.223,2.223,0,0,1,58.1,57.451a1.963,1.963,0,0,1,0-1.481A2.241,2.241,0,0,1,59.26,54.8a1.95,1.95,0,0,1,1.483,0,2.237,2.237,0,0,1,1.164,1.167,1.961,1.961,0,0,1,0,1.48,2.243,2.243,0,0,1-.469.689,2.266,2.266,0,0,1-.693.471,1.792,1.792,0,0,1-.741.146Z"/></g></g></g></svg>',
        10: '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 120 120"><defs><style>.cls-1{isolation:isolate;}.cls-2{fill:none;}.cls-3{fill:#8f1ae2;}.cls-4{opacity:0.4;fill:url(#무제_그라디언트_3);}.cls-4,.cls-5{mix-blend-mode:overlay;}.cls-5{fill:url(#무제_그라디언트_3-2);}.cls-6{fill:url(#무제_그라디언트_130);}.cls-7{fill:#fff;}.cls-8{fill:url(#무제_그라디언트_130-2);}</style><linearGradient id="무제_그라디언트_3" x1="60" y1="60" x2="60" y2="110" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#fff" stop-opacity="0.2"/><stop offset="1" stop-color="#fff"/></linearGradient><linearGradient id="무제_그라디언트_3-2" x1="60" y1="25.009" x2="60" y2="94.991" gradientTransform="translate(60 144.853) rotate(-135)" xlink:href="#무제_그라디언트_3"/><linearGradient id="무제_그라디언트_130" x1="46.56" y1="78.535" x2="46.56" y2="44.553" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#2a385f"/><stop offset="1" stop-color="#15202b"/></linearGradient><linearGradient id="무제_그라디언트_130-2" x1="69.986" y1="79.396" x2="69.986" y2="43.651" xlink:href="#무제_그라디언트_130"/></defs><title>c10</title><g class="cls-1"><g id="레이어_2" data-name="레이어 2"><g id="레이어_1-2" data-name="레이어 1"><rect class="cls-2" width="120" height="120"/><rect class="cls-3" x="24.645" y="24.645" width="70.711" height="70.711" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-4" points="10 60 60 110 110 60 10 60"/><rect class="cls-5" x="35.258" y="35.258" width="49.485" height="49.485" transform="translate(144.853 60) rotate(135)"/><polygon class="cls-6" points="43.261 78.535 43.261 54.432 38.751 54.432 38.751 44.553 54.369 44.553 54.369 78.535 43.261 78.535"/><path class="cls-7" d="M52.832,46.09V77H44.8V52.9h-4.51v-6.8H52.832m3.074-3.075H37.214V55.969h4.509v24.1H55.906V43.015Z"/><path class="cls-8" d="M70.007,79.4a13.536,13.536,0,0,1-5.982-1.324,13.683,13.683,0,0,1-4.662-3.735,17.317,17.317,0,0,1-2.973-5.664,25.309,25.309,0,0,1,0-14.341,17.066,17.066,0,0,1,2.978-5.648,13.758,13.758,0,0,1,4.658-3.709A13.654,13.654,0,0,1,80.632,48.69a17.288,17.288,0,0,1,2.952,5.643,25.329,25.329,0,0,1,0,14.34,17.556,17.556,0,0,1-2.948,5.657,13.426,13.426,0,0,1-4.672,3.744A13.581,13.581,0,0,1,70.007,79.4Zm0-25.538a2.018,2.018,0,0,0-1.235.388A4.207,4.207,0,0,0,67.6,55.635a9.078,9.078,0,0,0-.883,2.484,17.36,17.36,0,0,0,0,6.783,9.424,9.424,0,0,0,.891,2.5,4.158,4.158,0,0,0,1.168,1.4,2.024,2.024,0,0,0,1.235.388,2.08,2.08,0,0,0,1.229-.4,3.886,3.886,0,0,0,1.147-1.374,9.9,9.9,0,0,0,.879-2.515,17.447,17.447,0,0,0,0-6.785,9.573,9.573,0,0,0-.873-2.5,3.921,3.921,0,0,0-1.152-1.359A2.075,2.075,0,0,0,70.007,53.858Z"/><path class="cls-7" d="M70.007,45.188a11.856,11.856,0,0,1,9.428,4.468A15.724,15.724,0,0,1,82.121,54.8a23.827,23.827,0,0,1,0,13.4,15.972,15.972,0,0,1-2.686,5.165,11.847,11.847,0,0,1-4.14,3.32,12.072,12.072,0,0,1-14.736-3.32,15.7,15.7,0,0,1-2.7-5.165,23.779,23.779,0,0,1,0-13.4,15.456,15.456,0,0,1,2.7-5.145,12.2,12.2,0,0,1,4.14-3.3,11.952,11.952,0,0,1,5.308-1.168m0,25.538a3.6,3.6,0,0,0,2.112-.676,5.372,5.372,0,0,0,1.619-1.907,11.37,11.37,0,0,0,1.025-2.91,18.986,18.986,0,0,0,0-7.44,11.025,11.025,0,0,0-1.025-2.91A5.409,5.409,0,0,0,72.119,53a3.667,3.667,0,0,0-4.243,0,5.693,5.693,0,0,0-1.62,1.886,10.576,10.576,0,0,0-1.045,2.91,18.936,18.936,0,0,0,0,7.44,10.9,10.9,0,0,0,1.045,2.91,5.649,5.649,0,0,0,1.62,1.907,3.564,3.564,0,0,0,2.131.676m0-28.612a15.047,15.047,0,0,0-6.655,1.479,15.3,15.3,0,0,0-5.174,4.118,18.58,18.58,0,0,0-3.253,6.154,26.849,26.849,0,0,0,0,15.277,18.806,18.806,0,0,0,3.244,6.163,15.378,15.378,0,0,0,18.462,4.154,14.958,14.958,0,0,0,5.2-4.168,19.1,19.1,0,0,0,3.213-6.149,26.86,26.86,0,0,0,0-15.277,18.838,18.838,0,0,0-3.222-6.14,14.93,14.93,0,0,0-11.82-5.611Zm0,25.537a.513.513,0,0,1-.34-.1,2.658,2.658,0,0,1-.717-.891,7.876,7.876,0,0,1-.736-2.088,15.9,15.9,0,0,1,0-6.128,7.575,7.575,0,0,1,.725-2.064,2.706,2.706,0,0,1,.727-.886.5.5,0,0,1,.339-.1.534.534,0,0,1,.346.119,2.418,2.418,0,0,1,.685.838,8.093,8.093,0,0,1,.721,2.093,15.942,15.942,0,0,1,0,6.131,8.389,8.389,0,0,1-.736,2.123,2.384,2.384,0,0,1-.674.836.532.532,0,0,1-.343.116Z"/></g></g></g></svg>',
      }
      const tierSVGList = {
        I: '<path xmlns="http://www.w3.org/2000/svg" class="cls-22" d="M185.6,323.31V136.11l-47.52,36.72L106.76,130l83.85-61.56h55.11V323.31Z" transform="translate(0)"/>',
        II: '<path xmlns="http://www.w3.org/2000/svg" class="cls-22" d="M108.2,323.31V273l99.36-89.85a93.82,93.82,0,0,0,15.12-18.33A38.88,38.88,0,0,0,228.44,144q0-13.65-8.64-22.1t-23-8.45q-15.12,0-24.66,10.62T160.76,152l-58-7.92a91,91,0,0,1,10.62-34.62,90,90,0,0,1,21.42-25.89,92.61,92.61,0,0,1,29.7-16.31A110.86,110.86,0,0,1,200,61.59a125.53,125.53,0,0,1,34.2,4.68,89.24,89.24,0,0,1,29.52,14.4,71.88,71.88,0,0,1,20.7,24.48q7.74,14.77,7.74,34.56a78.88,78.88,0,0,1-3.6,24.66,81.62,81.62,0,0,1-9.9,20.34,116.69,116.69,0,0,1-14.4,17.46q-8.1,8.1-16.74,16l-59,52.56h104v52.56Z" transform="translate(0)"/>',
        III: '<path xmlns="http://www.w3.org/2000/svg" class="cls-22" d="M293.6,249.27q0,20.85-8.46,36.13A76,76,0,0,1,263,310.56a97.64,97.64,0,0,1-30.78,14.74,126.85,126.85,0,0,1-34.74,4.85,142.73,142.73,0,0,1-33.66-4,106.16,106.16,0,0,1-29.88-12.06,86.49,86.49,0,0,1-23.58-20.88q-10.08-12.76-15.12-30.78l56.16-14.76q3.6,12.61,14.76,22.14t28.44,9.54a49.79,49.79,0,0,0,13.14-1.8A35.92,35.92,0,0,0,219.62,272a30.07,30.07,0,0,0,8.64-9.9q3.42-6.11,3.42-15.12,0-9.72-4.5-16.2a31.87,31.87,0,0,0-11.88-10.26,58.34,58.34,0,0,0-16.74-5.4,109.58,109.58,0,0,0-18.72-1.62h-16.2V169.59h17.64a105.32,105.32,0,0,0,16.56-1.26,44.11,44.11,0,0,0,14.22-4.71,27.61,27.61,0,0,0,10.08-9.24q3.78-5.79,3.78-15.21,0-13.77-9.72-21a36.36,36.36,0,0,0-22.32-7.25,34.62,34.62,0,0,0-22.5,7.74A35.77,35.77,0,0,0,159,139.35l-56.16-13a83.31,83.31,0,0,1,14-28.26A86.36,86.36,0,0,1,139,78a102.2,102.2,0,0,1,27.9-12.24,117.12,117.12,0,0,1,31-4.14,126,126,0,0,1,32.94,4.33A88.14,88.14,0,0,1,259.4,79.26a70.38,70.38,0,0,1,20.34,22.53q7.74,13.53,7.74,31.91,0,21.28-12.06,35.88a58.73,58.73,0,0,1-30.78,19.64v1.08a65.79,65.79,0,0,1,19.8,8.09,63.92,63.92,0,0,1,15.48,13.31A59.22,59.22,0,0,1,290,229,57,57,0,0,1,293.6,249.27Z" transform="translate(0)"/>',
        IV: '<path xmlns="http://www.w3.org/2000/svg" class="cls-22" d="M269.12,273.63v49.68H212.6V273.63H92.72V222.87L196.4,68.43h72.36V224.67H304v49ZM213,130.35h-1.08L151.4,224.67H213Z" transform="translate(0)"/>',
        V: '<path xmlns="http://www.w3.org/2000/svg" class="cls-22" d="M293.24,239.07q0,23.42-8.28,40.5a80.79,80.79,0,0,1-22.14,28.26,95.4,95.4,0,0,1-31.5,16.74,122,122,0,0,1-69.3,1.08,105,105,0,0,1-28.44-12.78,86.12,86.12,0,0,1-22-20.34,81.21,81.21,0,0,1-13.5-27.18l55.08-16.92a42.82,42.82,0,0,0,14.94,22q11,8.64,26.1,8.64a40.76,40.76,0,0,0,26.82-9.36Q232.4,260.33,232.4,242q0-11.14-4.68-18.72a36.31,36.31,0,0,0-12.06-12.06,50.2,50.2,0,0,0-16.74-6.3,98.92,98.92,0,0,0-18.72-1.8,183.54,183.54,0,0,0-31.14,3.06,181.34,181.34,0,0,0-31.14,8.1L124,68.43H280.28v51.84H177.68l-2.16,40.32a78.33,78.33,0,0,1,12.78-2q7-.54,12.78-.54a125.49,125.49,0,0,1,36,5,82.55,82.55,0,0,1,29.34,15.3A73.33,73.33,0,0,1,286,203.79Q293.24,218.91,293.24,239.07Z" transform="translate(0)"/>',
      }
      classSVG = classSVGList[data1['class']]
      tier = tierList[data1['tier']]
      rank = data1['rank']
      rating = data1['rating']
      univ = []
      // for (const data of data2) {
      //   if (data['type'] == 'university') {
      //     univ = data['name']
      //   }
      // }
      // <!-- #ad5600 브론즈 #435f7a: 실버 #ec9a00: 골드 #27e2a4: 플레 #00b4fc: 다이아 #ff0062: 루비 #B491FF: 마스터 --->
      color = ''
      tierName = tier.split(' ')[0]
      tierSVG = tierSVGList[tier.split(' ')[1]]
      if (tierName == 'Bronze') color = '#ad5600'
      else if (tierName == 'Silver') color = '#435f7a'
      else if (tierName == 'Gold') color = '#ec9a00'
      else if (tierName == 'Platinum') color = '#27e2a4'
      else if (tierName == 'Diamond') color = '#00b4fc'
      else if (tierName == 'Ruby') color = '#ff0062'
      else if (tierName == 'Master') color = '#B491FF'
      infoNone = `
    <g transform="translate(320, 45)">
      <text class="user">ID</text>
    </g>
    <g transform="translate(390, 45)">
      <text class="user">${username}</text>
    </g>
    <g transform="translate(320, 65)">
      <text class="user">RANK</text>
    </g>
    <g transform="translate(390, 65)">
      <text class="user">${rank}</text>
    </g>
    <g transform="translate(320, 85)">
      <text class="user">RATING</text>
    </g>
    <g transform="translate(390, 85)">
      <text class="user">${rating}</text>
    </g>
    `
      infoUniv = `
    <g transform="translate(320, 37)">
      <text class="user">ID</text>
    </g>
    <g transform="translate(390, 37)">
      <text class="user">${username}</text>
    </g>
    <g transform="translate(320, 57)">
      <text class="user">RANK</text>
    </g>
    <g transform="translate(390, 57)">
      <text class="user">${rank}</text>
    </g>
    <g transform="translate(320, 77)">
      <text class="user">RATING</text>
    </g>
    <g transform="translate(390, 77)">
      <text class="user">${rating}</text>
    </g>
    <g transform="translate(320, 97)">
      <text class="user">UNIV</text>
    </g>
    <g transform="translate(390, 97)">
      <text class="user">${univ}</text>
    </g>
    `
      html = `
    <svg width="540" height="120" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 540 120">
    <!-- <link xmlns="http://www.w3.org/1999/xhtml" rel="stylesheet" href="test.css" type="text/css"/> -->
      <defs>
        <style>
        <![CDATA[
        svg svg {
          animation: fade 2s;
          filter: drop-shadow(40px 40px 40px ${color});
        }
        .tier {
          animation: fade 3s;
          font-size: 2em;
          fill: ${tier == 'Master' ? 'url(#master)' : color /*'url(#another)'*/};
          stroke: ${tier == 'Master' ? 'url(#master)' : color /*'url(#another)'*/};
          /* text-shadow: 1px 1px 2px ${color}; */
        }
        @keyframes fade {
          0%, 50% {
            opacity: 0;
          }
          100% {
            opacity: 1;
          }
        }
        .univ {
          animation: univ 4s;
          font-size: 1em;
          font-weight: bold;
          fill: ${color};
        }
        @keyframes univ {
          0%, 50% {
            fill: #00000000;
          }
          100% {
            fill: ${color};
          }
        }
        .user {
          animation: user 4s;
          font-size: 1em;
          font-weight: bold;
          fill: #ffffff;
        }
        @keyframes user {
          0%, 50% {
            fill: #00000000;
          }
          100% {
            fill: #ffffff;
          }
        }
        .card {
          animation: stroke 4s infinite alternate;
          fill: url(#card);
          stroke: ${color};
        }
        @keyframes stroke {
          0% {
            stroke: ${color};
            stroke-dashoffset: 30%;
            stroke-dasharray: 0 50%;
          }
          25% {
            stroke: ${color};
          }
          50% {
            stroke: ${color};
          }
          75% {
            stroke: ${color};
          }
          100% {
            stroke: ${color};
            stroke-dashoffset: -30%;
            stroke-dasharray: 50% 0;
            stroke-width: 3.0;
          }
        }
        ]]>
        </style>
      </defs>
      <defs>
        <linearGradient id="card">
          <stop offset="0%" stop-color="${bgc1}">
            <animate attributeName="stop-color" values="${bgc1}; ${bgc2}; ${bgc1};" dur="4s" repeatCount="indefinite"></animate>
          </stop>
          <stop offset="100%" stop-color="${bgc2}">
            <animate attributeName="stop-color" values="${bgc2}; ${bgc3}; ${bgc2};" dur="4s" repeatCount="indefinite"></animate>
          </stop>
        </linearGradient>  
      </defs>
      <rect class="card" x="10" y="10" rx="5" ry="5" width="520" height="100"/>
      <g transform="translate(25, 30)">
        <svg width="64" height="64" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 390 512">
          <defs>
            <style>.cls-11{fill:${color};}.cls-22{fill:#fff;}.st0{fill:url(#SVGID_1_);}.st1{fill:#FFFFFF;}</style>
            <linearGradient id="SVGID_1_" gradientUnits="userSpaceOnUse" x1="200" y1="512" x2="200" y2="4.325293e-06">
              <stop offset="0" style="stop-color:#FF7CA8"/>
              <stop offset="0.5" style="stop-color:#B491FF"/>
              <stop offset="1" style="stop-color:#7CF9FF"/>
            </linearGradient>
            <linearGradient id="master" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" style="stop-color:#7CF9FF; stop-opacity:1"/>
              <stop offset="50%" style="stop-color:#B491FF; stop-opacity:1"/>
              <stop offset="100%" style="stop-color:#FF7CA8; stop-opacity:1"/>
            </linearGradient>
            <linearGradient id="another" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" style="stop-color:#FFFFFF; stop-opacity:1"/>
              <stop offset="50%" style="stop-color:${color}; stop-opacity:1"/>
              <stop offset="100%" style="stop-color:${color}; stop-opacity:1"/>
            </linearGradient>
          </defs>
          <polygon class="${tierName == 'Master' ? 'st0' : 'cls-11'}" points="0 0 0 419.74 199.77 512 390 419.74 390 0 0 0"/>
          ${
            tierSVG == undefined
              ? '<path class="st1" d="M72,83.7h75.5l52.2,147.8h0.6l52.5-147.8H328v226.6h-49.9V136.5h-0.6l-59.5,173.8h-38.1l-57.3-173.8h-0.6   v173.8H72V83.7z"/>'
              : tierSVG
          }
          <polygon class="${tierName == 'Master' ? 'st1' : 'cls-22'}" points="0 339.02 0 378.94 199.77 471.2 390 378.94 390 339.02 199.77 431.28 0 339.02"/>
        </svg>
      </g>
      <g transform="translate(-4, -4)">
        <foreignObject width="48" height="48">
          ${classSVG}
        </foreignObject>
      </g>
      <text class="tier" x="200" y="57" text-anchor="middle" alignment-baseline="central">${tier}</text>
      <!-- <g transform="translate(120, 70)">
        <text class="tier">${tier}</text>
      </g> -->
      ${univ == '' ? infoNone : infoUniv}
    </svg>
    `
      console.log(req.originalUrl)
      res.setHeader('Content-Type', 'image/svg+xml')
      res.setHeader('Cache-control', 'public, max-age=' + 3600)
      // res.setHeader('Cache-control', 'public, max-age=' + 3600 * 60 * 6)
      res.status(200).send(html)
    } catch (error) {
      console.log(error)
      res.status(401).json({statusCode: 401, message: 'unknown username.'})
    }
  },
)
app.get('*', (req, res) => {
  res.status(404).json({statusCode: 404, message: 'unknown request.'})
})

http.createServer(app).listen(1235)
